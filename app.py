...
def admin_about():
    ...
    if request.method == 'POST':
        ...
        # Update shop info
        about.shop_name = request.form['shop_name']
        about.date_created = datetime.strptime(request.form['date_created'], '%Y-%m-%d').date()
...
def admin_delete_product(product_id):
    ...
    try:
        ...
        db.session.commit()
        log_admin_activity(f" [ADMIN DELETE_PRODUCT] Deleted product: {product.name}", 'product', product.id)
...
def log_admin_activity(action, target_type=None, target_id=None):
    try:
        ...
        socketio.emit('new_activity', {
            ...
            'device': activity.user_agent[:35] + '...' if len(activity.user_agent) > 35 else activity.user_agent
